...
//IMPORTANTE
// Importar API´s
/*COVECA 1*/ const { inserta_Coveca_1,update_status_Coveca_1 } = require('./api_coveca/api_coveca');
/*COVECA 2*/ const { inserta_Coveca_2,update_status_Coveca_2 } = require('./api_coveca_dos/api_coveca_dos');
...
/*COSIP*/ const {altaTicketCosip, updateTicketCosip, uploadImagesToCOSIP} = require('./api_cosip/api_cosip');

/*AS&E*/ const {altaTicketASSINCE, uptadeTicketASNICE, uploadFilesASNICE, updateNoTicketMSAtoASNICE, addNotesASNICE} = require('./api_assince/api_assince');
...
/*SOTIC*/ const { insertTicket_Sotic, calificaTicket_Sotic, updateTicket_Sotic } = require('./api_sotic/api_sotic');
...
exports.Insert_Ticket_Servicio = async (req,res) => {
    try {
        ...
        switch (req.body.DATA_INSERT.CAM_CSC_SERVICIO_SOLICITA) {
            case 24: //COVECA 1
                (async () => {
                    resultadoApiProvider = await inserta_Coveca_1(req.body.DATA_INSERT);
            ...
            case 2: //COVECA 2
                (async () => {
                    resultadoApiProvider = await inserta_Coveca_2(req.body.DATA_INSERT);
            ...
            case 36: //ESPECTROSCOPIA
                (async () => {
                    ...
                    if (resultadoApiProvider.isSuccessProvider == false) {
                        ...
                        ResultData = {success: true,message: `Exito en consumo con proveedor`,count: 1,JsonData: reqInsertaTicketMS, ResProvider: JSON.parse(removeCircularReferences(resultadoApiProvider))};
                    }
                    res.status(200);
                    res.send(ResultData);
                })();
            ...
            case 20: //AS&E AMERICAN SINCE
                (async () => {
                    ...
                    if (resultadoApiProvider.isSuccessProvider == false) {
                        ...
                        let resUpdateTicketANAM = await updateNoTicketMSAtoASNICE(reqInsertaTicketMS.idTicketInsert ,resultProviderDataParse)

                        console.log(resUpdateTicketANAM);
        ...
        __Request_Pool.query(query, function(error, resultReturn){
            if (error) {
                ...
                if (resultReturn.affectedRows[0] === 0) {
                    ...
                    if (req.body.DATA_WHERE.CAM_CSC_SERVICIO_SOLICITA == 20 && req.body.DATA_UPDATE.ESTATUS_TICKET_CSC == 57) {
                        (async () => {
                    ...
                    else if (req.body.DATA_WHERE.CAM_CSC_SERVICIO_SOLICITA == 38 && req.body.DATA_UPDATE.ESTATUS_TICKET_CSC == 203) {
                        (async () => {
                            resultadoApiProvider = await updateTicket_Sotic(req.body);
                            if (resultadoApiProvider) {
                                console.log(resultadoApiProvider);
                            } else {
                                console.log(resultadoApiProvider);
                    ...
                    if (req.body.DATA_UPDATE.TIC_CALIFICADO == 1) {
                        switch (req.body.DATA_WHERE.CAM_CSC_SERVICIO_SOLICITA) {
                            ...
                            case 24: //COVECA 1
                                (async () => {
                                    resultadoApiProvider = await update_status_Coveca_1(req.body.DATA_UPDATE,req.body.DATA_WHERE);
                                    if (resultadoApiProvider) {
                                        console.log(resultadoApiProvider);
                                    } else {
                                        console.log(resultadoApiProvider);
                            ...
                            case 2: //COVECA 2
                                (async () => {
                                    resultadoApiProvider = await update_status_Coveca_2(req.body.DATA_UPDATE,req.body.DATA_WHERE);
                                    if (resultadoApiProvider) {
                                        console.log(resultadoApiProvider);
                                    } else {
                                        console.log(resultadoApiProvider);
                            ...
                            case 38: //SOTIC
                                (async () => {
                                    resultadoApiProvider = await calificaTicket_Sotic(req.body, req.body.TIC_ID_EXTERNO_PROVEEDOR);
                                    if (resultadoApiProvider) {
                                        console.log(resultadoApiProvider);
                                    } else {
                                        console.log(resultadoApiProvider);
...
exports.Insert_Bitacora_Ticket = async (req,res) => {
    ...
    try {
        ...
        if (!req.body.idProveedor) {
            console.log('No se manda a ningun proveedor');
            
        } else {
            switch (req.body.idProveedor) {
                case 20: //AS&E AMERICAN SINCE
                    (async () => {
                        resultadoApiProvider = await addNotesASNICE(req.body);
                        if (resultadoApiProvider.isSuccessProvider == false) {
                            ResultData = {success: false,message: `Comunicación con proveedor ${resultadoApiProvider.providerName} no exitosa: EstatusCode: ${resultadoApiProvider.statusCode}`,count: 0,JsonData: {}, ResProvider: removeCircularReferences(resultadoApiProvider)};    
                            dataInsertError.SAMT_HIST_API_GENERAL_RESPONSE = removeCircularReferences(resultadoApiProvider);
                            let logInsertadoError = await insertaLogErrorProveedor(dataInsertError,dataBodyToSend);
                        } else{
                            let resultProviderDataParse = resultadoApiProvider.Detail;
                        }
                    })();
                break;

                default:
                    console.log(`No hacer nada ${req.body.idProveedor}`);
                break;
            }
...
exports.uploadImagesToCOSIP = async (req,res) => {
    try {
        let ExtraVars = JSON.parse(req.body.extraData);
        let lastResult = null;
        const results = [];
        ...
        const files = req.files;  // Cambia de req.file a req.files (es un array)
        ...
        for (const file of files) {
            const fileData = file.buffer;  // Aquí tienes el buffer de cada archivo
            const fileName = file.originalname;
            
            
            resultadoApiProvider = await uploadImagesToCOSIP(ExtraVars, fileData, file);
            if (resultadoApiProvider.isSuccessProvider == false) {
                lastResult  = {success: false,message: `Comunicación con proveedor ${resultadoApiProvider.providerName} no exitosa: EstatusCode: ${resultadoApiProvider.statusCode}`,count: 0,JsonData: {}, ResProvider: resultadoApiProvider};
            } else{
                results.push({
                    dt: resultadoApiProvider
                });
                lastResult  = {success: true,message: `Exito en consumo con proveedor`,count: 1, ResProvider: resultadoApiProvider};
            }       
        }
...
exports.Get_Reporte_Bitacoras = async (req, res) => {
    try {
        let query = null;
        var __Request_Pool = dbConfig.Get_Db_conexion(req.query.EMP_CLV_EMPRESA,req.query.Type); 

        
        query = `SELECT INFO_TICKET.TIC_CSCTICKET AS TICKET
        ,INFO_TICKET.TIC_NEWID AS TIC_NEWID
        ,INFO_TICKET.TIC_ID_EXTERNO_PROVEEDOR AS FOLIO_TICKET_PROVEEDOR
        ,INFO_TICKET.TIC_ESTATUS_SLA_ENUM
        ,SAMT_CAM_MESA_DE_AYUDA.CAM_MESA_IDIOMA1 AS MESA_DE_AYUDA
        ,SAMT_ESTATUS_TICKET.ESTATUS_TICKET_IDIOMA1 AS ESTATUS
        ,NIVEL1.TIPIFICA_IDIOMA1 AS CATEGORIA
        ,NIVEL2.TIPIFICA_IDIOMA1 AS CATEGORIA_NIVEL_2
        ,NIVEL3.TIPIFICA_IDIOMA1 AS CATEGORIA_NIVEL_3
        ,SAMT_TIPO_TICKET.TIPO_TICKET_IDIOMA1 AS TIPO_DE_TICKET
        ,SAMT_TIPO_PRIORIDAD_TICKET.TIPO_PRIORIDAD_IDIOMA1 AS PRIORIDAD
        ,SAMT_CAM_TIPO_SERVICIO.TIPO_SERVICIO_IDIOMA1 AS TIPO_DE_CANAL
        ,INFO_TICKET.TIC_SOLICITA
        ,INFO_TICKET.TIC_ATIENDE
        ,SAMT_REQUISICION_PROVEEDORES.REQUISICION_PROVEEDORES_NOMBRE AS UBICACION_ADUANA_ALMACEN
        ,SAMT_CLIENTES.CLIENTE_NOMBRE AS CLIENTE
        ,SAMT_PROYECTOS.PM_NOMBRE AS SERVICIO
        ,SAMT_CAM_SERVICIO.CAM_SERVICIO_NOMBRE AS PROVEEDOR
        ,SAMT_TIPO_SEVERIDAD.TIPO_SEVERIDAD_TICKET_IDIOMA1 AS SEVERIDAD
        ,CONCAT(INFO_EMPLEADO_SOL.EMPLEADO_NOMBREEMPLEADO ,' ',INFO_EMPLEADO_SOL.EMPLEADO_APATERNOEMPLEADO, ' ', INFO_EMPLEADO_SOL.EMPLEADO_AMATERNOEMPLEADO) AS NOM_EMP_SOL
        ,CONCAT(INFO_EMPLEADO_ATN.EMPLEADO_NOMBREEMPLEADO ,' ',INFO_EMPLEADO_ATN.EMPLEADO_APATERNOEMPLEADO, ' ', INFO_EMPLEADO_ATN.EMPLEADO_AMATERNOEMPLEADO) AS NOM_EMP_ATN
        ,INFO_EMPLEADO_ATN.EMPLEADO_ID_EXTERNO
        ,INFO_TICKET.TIC_EMAIL_SOLICITANTE
        ,INFO_TICKET.TIC_TELEFONO_SOLICITANTE
        ,INFO_TICKET.TIC_SEGMENTACION_INM_DES AS UBICACION
        ,INFO_TICKET.TIC_DESCRIPCION AS DESCRIPCION
        ,INFO_TICKET.TIC_DESCRIPCION_SOLUCION AS SOLUCION
        ,DATE_FORMAT(CONVERT_TZ(INFO_TICKET.TIC_FECHA_ALTA,'${req.query.TimeZoneServer}','${req.query.TimeZoneCliente}'),'%Y-%m-%d %H:%i:%s') AS FECHA_ALTA
        ,DATE_FORMAT(CONVERT_TZ(INFO_TICKET.TIC_FECHA_PROMESA,'${req.query.TimeZoneServer}','${req.query.TimeZoneCliente}'),'%Y-%m-%d %H:%i:%s') AS FECHA_PROMESA
        ,DATE_FORMAT(CONVERT_TZ(INFO_TICKET.TIC_FECHA_CIERRE,'${req.query.TimeZoneServer}','${req.query.TimeZoneCliente}'),'%Y-%m-%d %H:%i:%s') AS FECHA_CIERRE
        ,CASE 
        WHEN INFO_TICKET.TIC_CERRADO = TRUE THEN 'SI'
        WHEN INFO_TICKET.TIC_CERRADO = FALSE THEN 'NO'
        ELSE 'OTRO'
        END AS CERRADO
        ,CASE 
        WHEN INFO_TICKET.TIC_CALIFICADO = TRUE THEN 'SI'
        WHEN INFO_TICKET.TIC_CALIFICADO = FALSE THEN 'NO'
        ELSE 'OTRO'
        END AS CALIFICADO
        ,SAMT_TIPO_CALIFICACION.TIPO_CALIFICACION_IDIOMA1 AS ESTATUS_CALIFICACION
        ,INFO_TICKET.TIC_ID_EXTERNO_PROVEEDOR AS TICKET_EXTERNO
        ,INFO_TICKET.TAG_COMPONENTE AS TAG_COMPONENTE
        ,INFO_TICKET.TIC_CODIGO_VPN AS CODIGO_VPN
        ,CONCAT(
        FLOOR(TIMESTAMPDIFF(SECOND, INFO_TICKET.TIC_FECHA_ALTA, 
            CASE 
        WHEN TIC_CERRADO = 1 THEN INFO_TICKET.TIC_FECHA_CIERRE
        ELSE NOW()
            END
        ) / 86400), ' d ',
        TIME_FORMAT(SEC_TO_TIME(MOD(
            TIMESTAMPDIFF(SECOND, INFO_TICKET.TIC_FECHA_ALTA, 
        CASE 
            WHEN TIC_CERRADO = 1 THEN INFO_TICKET.TIC_FECHA_CIERRE
            ELSE NOW()
        END
        ...
        ,CASE WHEN INFO_TICKET.TIC_CERRADO = 0 THEN '00:00:00'
            WHEN
        (COALESCE(
        (
        SELECT  SUM(TIMESTAMPDIFF(SECOND, tl.LOG_FECHA_HORA, CASE WHEN tl.LOG_FECHA_HORA_FIN IS NULL AND INFO_TICKET.TIC_FECHA_CIERRE IS NULL THEN NOW() ELSE tl.LOG_FECHA_HORA_FIN END)) AS total_pause_duration
        ...
        ,0)) - INFO_TICKET.TIC_TIEMPO_EJECUCION < 0  THEN '00:00:00'
            ELSE
        ...
        ,0)) - INFO_TICKET.TIC_TIEMPO_EJECUCION)
        END AS TIEMPO_PENALIZACION

        ,CASE WHEN INFO_TICKET.TIC_CERRADO = 1 THEN
        CASE WHEN (COALESCE(
        (
        SELECT  SUM(TIMESTAMPDIFF(SECOND, tl.LOG_FECHA_HORA, CASE WHEN tl.LOG_FECHA_HORA_FIN IS NULL AND INFO_TICKET.TIC_FECHA_CIERRE IS NULL THEN NOW() ELSE tl.LOG_FECHA_HORA_FIN END)) AS total_pause_duration
        FROM SAMT_TICKET_LOG_SLA tl
        ...
        ,0)) > INFO_TICKET.TIC_TIEMPO_EJECUCION THEN 'Resuelto fuera de SLA'
        ELSE 'Resuelto en tiempo' END
        ELSE
            CASE WHEN IFNULL(TIC_FECHA_PROMESA, NOW()) < NOW() THEN 'Vencido'
        WHEN IFNULL(TIC_FECHA_PROMESA, NOW()) > NOW() THEN 'En Tiempo'
            ELSE 'En Tiempo' END
        END AS CUMPLIMIENTO_SLA,

            GROUP_CONCAT(  
            CONCAT(DATE_FORMAT(CONVERT_TZ(SAMT_TICKET_BITACORA.TIB_FECHAHORA, 'Etc/GMT', 'America/Mexico_City'), '%Y-%m-%d %H:%i:%s'),
            ' - ',
            SAMT_TICKET_BITACORA.TIB_DESCRIPCION)
        SEPARATOR ' | ') AS BITACORA

        ,INFO_TICKET.CAM_CSC_SERVICIO_SOLICITA
        ,INFO_TICKET.TIC_TIEMPO_RECEPCION
        ,INFO_TICKET.TIC_DURACION_RECEPCION
        ,INFO_TICKET.TIC_FECHA_ALTA
        ,INFO_TICKET.TIC_TIEMPO_EJECUCION
        ,INFO_TICKET.TIC_FECHA_CIERRE
        ,INFO_TICKET.CAM_MESA_CSC
        ...
        ,INFOEQUIPO.EQUIPAMIENTO_REFERENCIA
		,INFOEQUIPO.EQUIPAMIENTO_NO_SERIE
        ,INFOEQUIPO.EQUIIPAMIENTO_ALIAS
        FROM SAMT_TICKET_SERVICIO AS INFO_TICKET
        LEFT JOIN SAMT_EMPLEADOS AS INFO_EMPLEADO_ATN 
        ON INFO_EMPLEADO_ATN.EMPLEADO_CSC_EMPLEADO = INFO_TICKET.EMPLEADO_CSC_ATIENDE 
        AND INFO_EMPLEADO_ATN.EMP_CSC_EMPRESA_HOST = INFO_TICKET.EMP_CSC_EMPRESA_HOST

        LEFT JOIN SAMT_EMPLEADOS AS INFO_EMPLEADO_SOL 
        ON INFO_EMPLEADO_SOL.EMPLEADO_CSC_EMPLEADO = INFO_TICKET.EMPLEADO_CSC_SOLICITA 
        AND INFO_EMPLEADO_SOL.EMP_CSC_EMPRESA_HOST = INFO_TICKET.EMP_CSC_EMPRESA_HOST

        LEFT JOIN SAMT_REQUISICION_PROVEEDORES
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_REQUISICION_PROVEEDORES.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.REQ_CSCREQUISICION = SAMT_REQUISICION_PROVEEDORES.SAMT_REQUISICION_PROVEEDORES_CSC

        LEFT JOIN SAMT_CAM_MESA_DE_AYUDA
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_CAM_MESA_DE_AYUDA.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.CAM_MESA_CSC = SAMT_CAM_MESA_DE_AYUDA.CAM_MESA_CSC 

        LEFT JOIN SAMT_CAM_TIPIFICACIONES AS NIVEL1
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = NIVEL1.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.TIPIFICA_CSC_PARENT_PARENT = NIVEL1.TIPIFICA_CSC

        LEFT JOIN SAMT_CAM_TIPIFICACIONES AS NIVEL2
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = NIVEL2.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.TIPIFICA_CSC_PARENT = NIVEL2.TIPIFICA_CSC

        LEFT JOIN SAMT_CAM_TIPIFICACIONES AS NIVEL3
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = NIVEL3.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.TIPIFICA_CSC = NIVEL3.TIPIFICA_CSC

        LEFT JOIN SAMT_TIPO_TICKET
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_TIPO_TICKET.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.TIPO_TICKET_CSC = SAMT_TIPO_TICKET.TIPO_TICKET_CSC 

        LEFT JOIN SAMT_TIPO_PRIORIDAD_TICKET
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_TIPO_PRIORIDAD_TICKET.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.TIPO_PRIORIDAD_CSC = SAMT_TIPO_PRIORIDAD_TICKET.TIPO_PRIORIDAD_CSC 

        LEFT JOIN SAMT_ESTATUS_TICKET
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_ESTATUS_TICKET.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.ESTATUS_TICKET_CSC = SAMT_ESTATUS_TICKET.ESTATUS_TICKET_CSC 

        LEFT JOIN SAMT_CAM_TIPO_SERVICIO
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_CAM_TIPO_SERVICIO.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.SAMT_CAM_TIPO_SERVICIO_CSC = SAMT_CAM_TIPO_SERVICIO.SAMT_CAM_TIPO_SERVICIO_CSC 

        LEFT JOIN SAMT_CLIENTES
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_CLIENTES.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.CLIENTE_CSC_SOLICITA = SAMT_CLIENTES.CLIENTE_CSC 

        LEFT JOIN SAMT_PROYECTOS
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_PROYECTOS.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.PM_CSC_PROYECTO_SOLICITA = SAMT_PROYECTOS.PM_CSC_PROYECTO 

        LEFT JOIN SAMT_CAM_SERVICIO
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_CAM_SERVICIO.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.CAM_CSC_SERVICIO_SOLICITA = SAMT_CAM_SERVICIO.CAM_CSC_SERVICIO 

        LEFT JOIN SAMT_TIPO_SEVERIDAD
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_TIPO_SEVERIDAD.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.TIPO_SEVERIDAD_CSC = SAMT_TIPO_SEVERIDAD.TIPO_SEVERIDAD_CSC 

        LEFT JOIN SAMT_TIPO_CALIFICACION
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_TIPO_CALIFICACION.EMP_CSC_EMPRESA_HOST 
        AND INFO_TICKET.TIPO_CALIFICACION_CSC = SAMT_TIPO_CALIFICACION.TIPO_CALIFICACION_CSC 

        LEFT JOIN SAMT_EMPLEADOS AS INFO_EMP_ALTA
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = INFO_EMP_ALTA.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.AUDITORIA_USU_ALTA = INFO_EMP_ALTA.EMPLEADO_CSC_EMPLEADO 

        LEFT JOIN SAMT_EMPLEADOS AS INFO_EMP_MODIFICA
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = INFO_EMP_MODIFICA.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.AUDITORIA_USU_ALTA = INFO_EMP_MODIFICA.EMPLEADO_CSC_EMPLEADO 

        LEFT JOIN SAMT_TICKET_TIPO_URGENCIA
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_TICKET_TIPO_URGENCIA.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.SAMT_TICKET_TIPO_URGENCIA_CSC = SAMT_TICKET_TIPO_URGENCIA.SAMT_TICKET_TIPO_URGENCIA_CSC 

        LEFT JOIN SAMT_TICKET_BITACORA 
        ON INFO_TICKET.EMP_CSC_EMPRESA_HOST = SAMT_TICKET_BITACORA.EMP_CSC_EMPRESA_HOST
        AND INFO_TICKET.TIC_CSCTICKET = SAMT_TICKET_BITACORA.TIC_CSCTICKET

        LEFT JOIN SAMT_EQUIPO AS INFOEQUIPO ON 
        INFOEQUIPO.CAM_CSC_SERVICIO = INFO_TICKET.CAM_CSC_SERVICIO_SOLICITA AND INFOEQUIPO.EQUIPAMIENTO_HOSTNAME = INFO_TICKET.TAG_COMPONENTE
        ...
        query += "GROUP BY INFO_TICKET.TIC_CSCTICKET ORDER BY INFO_TICKET.TIC_CSCTICKET DESC;--";
        __Request_Pool.query(query,function(error, resultReturn, fields){
            if (error) {
                ResultData = {success: false,message: error.message};
                res.status(400);
...
exports.Get_Reporte_dgmeia = async (req, res) => {
    try {
        let query = null;
        let __Request_Pool = dbConfig.Get_Db_conexion(req.query.EMP_CLV_EMPRESA, req.query.Type);
        ...
        let fechaInicio = null;
        let fechaFin = null;
        let idMesaServicio = null;

        if (req.query.FECHA_INICIAL === 'NULL') {
            fechaInicio = 'NULL';
        } else {
            fechaInicio = __Request_Pool.escape(req.query.FECHA_INICIAL);
        }

        if (req.query.FECHA_FINAL === 'NULL') {
            fechaFin = 'NULL';
        } else {
            fechaFin = __Request_Pool.escape(req.query.FECHA_FINAL);
        }

        if (req.query.ID_MESA_SERVICIO === 'NULL') {
            idMesaServicio = 'NULL';
        } else {
            idMesaServicio = __Request_Pool.escape(req.query.ID_MESA_SERVICIO);
        }

         query = `call sp_reporte_dgmeia(
            ${req.query.EMP_CSC_EMPRESA_HOST}
            ,${idMesaServicio}
            ,${fechaInicio}
            ,${fechaFin});`;
