...
const url_Provider = process.env.ESPECTROSCIPIA_URL;
const fs = require('fs');
const path = './dataTokenProvider.AssertiveEspectroscipia.json';
...
async function altaTicketEspectroscipia(data = null,stringsData = null) {
    ...
    objAlta.tipo = stringsData.tipi_1;
    ...
    const success = await consumirApi_AmericanSince(`${url_Provider}/nuevo`, 'POST', { key: `${resultLogout.Detail}` }, objAlta, "ESPECTROSCOPIA");
    console.log(success);
    return success; // Retornar el resultado de la llamada
}

async function updateTicketEspectroscopia(data = null, TIC_ID_EXTERNO_PROVEEDOR = null) {
    const currentData = readData();
    let resultLogout = await logOutAsertive(currentData.TokenAssertive);
    console.log(resultLogout);
            ...
            const success = await consumirApi(`${url_Provider}/evento/${TIC_ID_EXTERNO_PROVEEDOR}`, 'POST', { key: `${resultLogout.Detail}` }, objAlta, "ESPECTROSCOPIA");
            return success; // Retornar el resultado de la llamada
}
async function loginAsertive() {
    const urlLoginAsertive = process.env.ESPECTROSCIPIA_URL_LOGIN_LOGOUT+'/login'
    const objAlta = {}
    // Llenar las propiedades del objeto
    objAlta.email = process.env.ESPECTROSCIPIA_USERNAME;
    objAlta.pass = process.env.ESPECTROSCIPIA_PASSWORD;

    const success = await consumirApiNoTokens(`${urlLoginAsertive}`, 'POST', objAlta, "ESPECTROSCOPIALogIn");    
    updateData('TokenAssertive', success.Detail);
    return success; // Retornar el resultado de la llamada
}

async function logOutAsertive(tokenActivo) {
    const urlLoginAsertive = process.env.ESPECTROSCIPIA_URL_LOGIN_LOGOUT+'/logout'
    const objAlta = {}
    // Llenar las propiedades del objeto
    objAlta.email = process.env.ESPECTROSCIPIA_USERNAME;
    objAlta.pass = process.env.ESPECTROSCIPIA_PASSWORD;
    ...
    let successLogout = await consumirApi(`${urlLoginAsertive}`, 'GET', { key: `${tokenActivo}` }, objAlta, "ESPECTROSCOPIALogOut");
...
module.exports = { altaTicketEspectroscipia, updateTicketEspectroscopia};