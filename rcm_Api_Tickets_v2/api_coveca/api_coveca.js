...
const Token_Provider = process.env.COVECA_TOKEN;
const url_Provider = process.env.COVECA_URL;

async function inserta_Coveca_1(data = null) {
    const objAlta = {}
    ...
    if (data.TIPO_TICKET_CSC == 30) {//Requerimiento
        objAlta.tipo_servicio = "230610125247";
    } else if(data.TIPO_TICKET_CSC == 31){ //Incidente
        objAlta.tipo_servicio = "230610125220";

    }
    objAlta.cod_equipo = data.TIC_JSON_EXTRA_PARAMS
    const success = await consumirApi(`${url_Provider}/Servicios`, 'POST', { Authorization: `Bearer ${Token_Provider}` }, objAlta, "COVECA 1");
    return success; // Retornar el resultado de la llamada
}

async function update_status_Coveca_1(data = null, dataWhere=null) {
    ...
    const success = await consumirApi(`${url_Provider}/UpdateSrv`, 'PUT', { Authorization: `Bearer ${Token_Provider}` }, objAlta, "COVECA 1");
    return success; // Retornar el resultado de la llamada
}

// Exportar la función
module.exports = { inserta_Coveca_1,update_status_Coveca_1};